...
export const Header = (props: { brandName: string; routes: { name: string; path: string }[] }) => {
  ...
  return (
    <header className="fixed top-0 w-full  text-white py-4 px-12 flex justify-between items-center z-50 hover:cursor-default">
        ...
        <ul className="flex space-x-4">
          {routes.map((route) => (
            <li key={route.name} className="group">
              <a className="text-lg font-bold cursor-pointer" href={route.path}>
                {route.name}
              </a>
